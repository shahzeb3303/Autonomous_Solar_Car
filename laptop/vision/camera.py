...
class Camera:
    ...
    def start(self) -> bool:
        # Force the V4L2 backend. The default backend can fall back to FFMPEG,
        # which mis-counts USB webcams and fails to open higher indices (e.g. the
        # external AUKEY at /dev/video4) with "index out of range".
        self.cap = cv2.VideoCapture(self.device, cv2.CAP_V4L2)
        if not self.cap.isOpened():
            self.cap = cv2.VideoCapture(self.device)   # fallback to default backend
        if not self.cap.isOpened():
